...
from ..models import Messages, User, Categorie
from ..serializers import MessagesSerializer, UserSerializer, CategorieSerializer
...
def login(request):
    try:
        ...
        if '@' in username_or_email:
            ...
            user = User.objects.get(username=username_or_email, password=password)
...
def update_user(request, user):
    ...
    if request.data.get('phone_number'):
        user.phone_number = validate_phone_number(request.data['phone_number'])
    user.save()
    serializer = UserSerializer(user, many=False)
    return Response(serializer.data)
...
def users_updates(request, pk=0):
    try:
        ...
        if not User.check_user_token(pk, None, request):
            raise Exception('User token or user.id invalid')
...
def get_user_categories(request, user_id):
    User.check_user_token(user_id, request=request)
